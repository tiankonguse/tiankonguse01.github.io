...
是的，编程语言中所有的坑都是代码不规范导致的。  

但是你想让所有人的代码都规范起来是很难得。  

比如我在给大一新生讲 C 语言时多次强调所有的需要加大括号的地方都要加大括号，即使只有医疗语句也要加，但是有几个人做得到呢？  
...
但是这个错误时由于我们没有的代码不规范导致的，我们通过加大括号可以解决。  
...
上面两个代码看起来优美多了，但是优美归优美，我们还是需要把所有问题解决了才算真正的优美。  



那对于 这个丑陋的代码 `{(a+1)*(b+1);};` 到底该如何解决呢？  

这么丑陋的代码是有这个语言本身的漏洞导致的，所以只好从语言本身上解决了。  
...
这和很多解释性语言类似，最后一条语句的返回值作为返回值函数的返回值。  

而这里大概意思是混合的语句使用括号包起来后可以作为一个表达式


这个问题颇为复杂，不过前端时间我写了这么一篇文章 [宏 do{}while(0)][do-while].  
