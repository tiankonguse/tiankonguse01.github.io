...
站在巨人的肩膀上可以更快的达到目标, 于是我向REDIS DB同事咨询他们针对大内存多核CPU是不是做了参数调优.  
结果他们回答REDIS部署多实例, 每个实例使用一个核, 使用2~4G内存.  
...
比如下面的信息可以看出有两个处理器, 每个处理器有12个核, 然后都开了24超线程, 所以对于用户总共就是48个核了.    
另一个信息就是每个处理器有自己的三级缓存.    
...
查询可以看到共有16个内存槽, 其中使用了8个, 每个内存16G, 合起来就是128G.    
...
于是硬件开发商就把内存控制器做了拆分, 内存插槽分别挂在部分CPU上.    
...
* 当程序的热点内存大小切好是某些大小时, 性能会急剧下降, 因为恰好遇到了N路关联(N-way set associative cache)的竞争.    
...
![](http://tiankonguse.com/lab/cloudLink/baidupan.php?url=/1915453531/3546716773.png)  
