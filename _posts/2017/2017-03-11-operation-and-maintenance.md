...
随着业务的发展, UNION系统的接入来源越来越多, 系统的访问量越来越大, 很多问题都逐渐保留出来.  
...
比如视频2001表有标题(title), 子标题(subtitle),播放量(playcount),设置(setting)等字段, UNION会有一张配置表配置这个表的字段该去哪个数据源拉数据.  
但是对于setting的含义是设置什么的, 什么场景下使用等, UNION其实是不知道的.  
...
于是我的世界观也逐渐的变化了: UNION系统就是我的一个产品, 我的用户没有自己想的那么聪明.  
