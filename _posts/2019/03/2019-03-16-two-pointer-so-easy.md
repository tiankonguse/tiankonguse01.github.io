...
layout:     post  
title:  【算法】双指针就是这么简单  
description: 有些算法需要使用两个指针才能解决，看了这篇文章，你会发现特别简单。  
keywords: 算法  
tags: [算法]    
categories: [算法]  
...
等于了则找到答案了，结束。小于了则代表选择的这个数字没有答案，迭代到下个数字去。  
...
对于马上覆盖的方法，说出来就很容易理解。  
当前元素直接和最后一个元素交换，然后数组的大小减一，然后重新从当前位置进行判断计算。  
...
在这两个方法里，都可以看到在最后有一个特殊判断。这是由于数组可能以`1`结束，最后一个答案就没有进行计算。  
...
面对那么暴露的方案，我们其实位置确定后，目标是快速找到满足条件的结束位置。  
那能不能使用二分呢？  
这里是检查子数组的和，每次子数组的起始位置不一样。  
但是我们发现，对于确定的一次查找，那些子数组的和都是前缀数组和减去固定起始位置的前缀数组`presum`。  
于是问题就可以转化为：求前缀数组和大于`S+presum`的最小位置。  
而对于前缀数组和，是可以预处理求出所有的前缀数组和，然后二分的。  
