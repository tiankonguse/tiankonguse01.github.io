...
第二篇《[一致性hash基础知识(二)](https://mp.weixin.qq.com/s/N4SKpKkeUUqajoQTIv96xw)》则介绍了在项目实际使用一致性HASH的时候，尤其是在高并发与高可靠的场景下，会面临哪些问题。  
...
我们的一致性HASH真实上线后，往往会发现每台机器的请求量并不是均衡的。  
...
那毫无疑问，我将所有的一致性HASH环调整为了`1000`个节点，之前困扰我很久的不均衡问题初步得到解决。  
...
此时我们怎么增大虚拟节点个数都是无效的。  
因为一个KEY只会路由到一个机器，而这个KEY的量大到那台机器支持不住时，那只能想其他办法了。  
...
含义是一个小set是一个一致性HASH环，能支持指定指标的流量的最小单位。  
...
这种架构是一种可无限扩容的设计，对当初提出这种设计的人致以感谢。  
当然，这里面存在维护成本。  


每一个小set都有自己的路由表，由于某些原因小set的机器还需要知道自己属于哪个大set时，配置表将更为复杂。  
如果服务的发布系统是自己做的话，可以集成这些功能，从而做到高效维护管理。  
...
比如30个KEY平均的均摊在5个节点上，对于每个节点的6个KEY可以一次请求去拉数据，而不是6次请求。  
但是这样做对内的QPS也有50W/S，其实这个量也不小。  


此时，当负载过高时，我们不能增加一致性HASH节。  
为什么呢？  
假设5台机器，对外QPS是10W/s，对内聚合请求QPS是50W/s。  
...
PS：昨天(3-26)，我有一个SET的命中率特别高，单机即可储存下。我尝试较少节点，CPU下降不少。  
