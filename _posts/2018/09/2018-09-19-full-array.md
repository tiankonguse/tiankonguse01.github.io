...
由于队列涉及到数组前面删除元素，这里可以使用一个标记来标记前面删除了多少个元素。  
...
不过这里会有一个问题，队首也可以插入数据了，有可能前面的空间用完了。  
...
问题1：队列的队尾没空间了，队首可能还有空间，我们没办法用起来，需要扩大内存。  
